...
public class LinePagerIndicatorDecoration extends RecyclerView.ItemDecoration {
    ...
    private int colorActive = Color.parseColor("#F69A4D");
    private int colorInactive = Color.parseColor("#C4C4C4");
    ...
    public void onDrawOver(Canvas canvas, RecyclerView recyclerView, RecyclerView.State state) {
        ...
        float f = (((float) this.mIndicatorHeight) / 2.0f) + 0.0f;
        drawInactiveIndicators(canvas, width, f, itemCount);
        LinearLayoutManager linearLayoutManager = (LinearLayoutManager) recyclerView.getLayoutManager();
        int findFirstVisibleItemPosition = linearLayoutManager.findFirstVisibleItemPosition();
        if (findFirstVisibleItemPosition != -1) {
            View findViewByPosition = linearLayoutManager.findViewByPosition(findFirstVisibleItemPosition);
            drawHighlights(canvas, width, f, findFirstVisibleItemPosition, this.mInterpolator.getInterpolation(((float) (findViewByPosition.getLeft() * -1)) / ((float) findViewByPosition.getWidth())), itemCount);
        }
    }
